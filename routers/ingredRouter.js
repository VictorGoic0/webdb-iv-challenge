...
router.get("/", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: `Ingredients could not be found ${error}.` });
  }
});
...
router.get("/:id", async (req, res) => {
  ...
  try {
    ...
    if (ingredients) {
      ...
      res
        .status(404)
        .json({ message: "ingredients with specified ID does not exist." });
    }
  } catch (error) {
...
router.post("/", async (req, res) => {
  ...
  if (!ingredients.name) {
    ...
    try {
      ...
      res
        .status(500)
        .json({ message: `Your ingredients could not be posted ${error}.` });
    }
  }
});
...
router.delete("/:id", async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({
      message: `The ingredients's information could not be modified: ${error}.`
    });
  }
});
